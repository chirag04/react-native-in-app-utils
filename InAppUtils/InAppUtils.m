   ...
   for (SKPaymentTransaction *transaction in transactions) {
       switch (transaction.transactionState) {
           case SKPaymentTransactionStateFailed: {
               ...
               if (callback) {
                   NSLog(@"Call back from updatedTransactions: %@", transaction.error);
                   callback(@[translateErrorCode(transaction.error.code, @"restore_failed")]);
...
restoreCompletedTransactionsFailedWithError:(NSError *)error
{
   ...
   RCTResponseSenderBlock callback = _callbacks[key];
   if (callback) {
       callback(@[translateErrorCode(error.code, @"restore_failed")]);
       [_callbacks removeObjectForKey:key];
   } else {
...
static NSString *translateErrorCode(NSInteger code, NSString* defaultError)
{
   switch (code)
   {
...
- (void)request:(SKRequest *)request didFailWithError:(NSError *)error{
   ...
   RCTResponseSenderBlock callback = _callbacks[key];
   if(callback) {
       NSLog(@"Callback from didFailWithError: %@", error);
       callback(@[translateErrorCode(error.code, @"unknown_error")]);
       [_callbacks removeObjectForKey:key];
   }
}
